...
class Admin extends CI_Controller
{
    ...
    public function clarans()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $numlocal = 2;
            $number_cluster = 2;
            $max_neighbor = $number_cluster * (count($data['list']) - $number_cluster);
            // $max_neighbor = 9;
            $min_cost = 9999;
            ...
            for ($i=1; $i<=$numlocal; $i++) { 
                ...
                if($cost_current<$min_cost){
                    ...
                    // $ax = $cost_class1;
                    // $bx = $cost_all_current2 - $cost_class2;
                    if ($cost_class1 < 1) {
                        $sx = 0;
                    }else {
                        $sx = (($cost_all_current2 - $cost_class2) - $cost_class1) / max($cost_class1,($cost_all_current2 - $cost_class2));
                    }
    ...
    public function displayAccess()
    {
        ...
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['class1'] = $this->menu->displayClass1();
        $data['class2'] = $this->menu->displayClass2();
    ...
    public function selectTransaction($transaction_id)
    {
        ...
        $data['transaction'] = $this->db->get_where('transaction', ['transaction_id' => $transaction_id])->row_array();
